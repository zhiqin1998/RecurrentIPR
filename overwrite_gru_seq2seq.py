...
from trainer.nmt_seq2seq import EncDecPrivateTrainer, EncDecEvaluator
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, dest='epochs', default=2, help='number of epochs')
    ...
    # remove sign loss from model
    encoder.gru.sign_loss = None
    decoder.gru.sign_loss = None

    old_enc_key = encoder.key.cpu().clone()
    old_dec_key = decoder.key.cpu().clone()

    # replace with new key
    if 'overwrite_enc_key' in keyed_kwargs:
        keyed_kwargs['enc_key'] = keyed_kwargs.pop('overwrite_enc_key')
        keyed_kwargs['dec_key'] = keyed_kwargs.pop('overwrite_dec_key')
    else:
        # prevent repeating key due to same seed
        keyed_kwargs['enc_key'] = torch.randint(max(0, num_words - 5000), num_words, (8, 15))
        keyed_kwargs['dec_key'] = torch.randint(max(0, num_words_outputs - 5000), num_words_outputs, (8, 15))
        keyed_kwargs['enc_key'] = torch.randint(max(0, num_words - 5000), num_words, (8, 15))
        keyed_kwargs['dec_key'] = torch.randint(max(0, num_words_outputs - 5000), num_words_outputs, (8, 15))

    print('new encoder key:')
    print(sequence_to_text(keyed_kwargs['enc_key'], src_vocab))
    print('\nnew decoder key:')
    print(sequence_to_text(keyed_kwargs['dec_key'], trg_vocab))

    encoder.set_key(keyed_kwargs['enc_key'].to(device))
    decoder.set_key(keyed_kwargs['dec_key'].to(device))
    ...
    encoder.set_key(old_enc_key.to(device))
    decoder.set_key(old_dec_key.to(device))
